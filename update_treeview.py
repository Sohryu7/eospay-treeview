...
FIELDS = [
    ...
    "customfield_10028",  # Story Points (alternative field)
]
...
def parse_issue(raw: dict) -> dict:
    ...
    priority = priority_obj.get("name", "")  # e.g. "High", "Medium", "Low"

    # Story Points (try both common Jira Cloud fields)
    story_points = f.get("customfield_10016") or f.get("customfield_10028") or None
...
def prepare_data(all_issues: list) -> dict:
    ...
    sp_stories = [s for s in all_issues if s["type"]=="Story" and s.get("story_points") is not None]
    print(f"  Stories mit Story Points:   {len(sp_stories)}")
...
.prio{{font-size:9px;font-weight:700;padding:1px 5px;border-radius:4px;flex-shrink:0;letter-spacing:.3px}}
.prio-highest{{background:#fecaca;color:#991b1b}}
...
.prio-lowest{{background:#f3f4f6;color:#6b7280}}
.sp{{font-size:10px;color:#6b7280;background:#f3f4f6;padding:1px 6px;border-radius:4px;flex-shrink:0;font-family:'SF Mono',monospace}}
.sp-sum{{padding:8px 12px;background:#fffbeb;border:1px solid #fcd34d;border-radius:8px;font-size:12px;color:#78350f;margin-bottom:10px;display:flex;gap:16px;flex-wrap:wrap}}
.sp-sum strong{{font-size:14px}}
</style>
</head>
<body>
...
<div class="sp-sum" id="sp-sum" style="display:none"></div>
<div id="tree"></div>
<script>
...
function prioBadge(p){{
  if(!p)return '';
  const cls={{Highest:'prio-highest',High:'prio-high',Medium:'prio-medium',Low:'prio-low',Lowest:'prio-lowest'}}[p]||'prio-lowest';
  return `<span class="prio ${{cls}}">${{p}}</span>`;
}}
function spBadgeStory(sp){{
  if(sp===null||sp===undefined||sp==='')return '';
  const val=sp%1===0?sp:sp.toFixed(1);
  return `<span class="sp">${{val}} SP</span>`;
}}
function matchesPriority(i,pf){{return !pf||(i.priority||'')==pf;}}
...
function subtaskHtml(st,q,sf,spf,pf,cnt){{
  ...
  h+=`<span class="bst">Sub-Task</span><span class="ikey"><a href="${{jiraLink(st.key)}}" target="_blank" onclick="event.stopPropagation()">${{st.key}}</a></span><div class="in"><div>${{st.summary}}</div></div>${{prioBadge(st.priority)}}<span class="pill ${{sc(st.status)}}">${{sl(st.status)}}</span>${{hasDeps?'<span style="color:#aaa;font-size:12px">&#128279;</span>':''}}</div>`;
...
function storyHtml(s,q,sf,spf,pf,cnt){{
  ...
  const sVis=showTypes.Story&&!spf&&idMatch&&matchesStatus(s,sf)&&!isDokuCancelled&&matchesPriority(s,pf);
  if(!sVis&&subtaskChildren.length===0&&devDirectChildren.length===0)return '';
  cnt.s++;
  // SP sum: count open stories with story_points
  if(s.story_points!==null&&s.story_points!==undefined&&s.status!=='Fertig'&&s.status!=='Abgebrochen'){{
    spTotal+=s.story_points; spCount++;
  }}
...
function render(){{
  ...
  const spDiv=document.getElementById('sp-sum');
  if(spCount>0){{
    const spVal=spTotal%1===0?spTotal:spTotal.toFixed(1);
    spDiv.style.display='flex';
    spDiv.innerHTML=`<span>&#x1F4CA; <strong>${{spVal}}</strong> Story Points offen</span><span style="color:#92400e;font-size:11px">(${{spCount}} Stories mit SP-Schätzung, Status ≠ Fertig/Abgebrochen)</span>`;
  }} else {{
